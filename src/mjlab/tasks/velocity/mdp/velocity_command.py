...
class RelativeHeightCommand(CommandTerm):
  ...
  def _debug_vis_impl(self, visualizer: "DebugVisualizer") -> None:
    ...
    actual_end = np.array([base_pos_w[0], base_pos_w[1], base_pos_w[2]], dtype=np.float32)
    ...
    visualizer.add_arrow(
      start, target_end, color=self.cfg.viz.target_color, width=0.015, label="height_target"
    )
    visualizer.add_arrow(
      start, actual_end, color=self.cfg.viz.actual_color, width=0.015, label="height_actual"
    )
