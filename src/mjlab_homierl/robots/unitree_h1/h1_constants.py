...
H1_XML: Path = PACKAGE_DIR / "xmls" / "h1.xml"
...
def _disable_geom_contact(geom: mujoco.MjsGeom) -> None:
  geom.contype = 0
  geom.conaffinity = 0


def _disable_attached_hand_collisions(model: mujoco.MjSpec, side: str) -> None:
  """Disable collisions for hand placeholders and attached gripper geoms.

  HOMIE uses the grippers as inertial/disturbance attachments, not as manipulation
  contact tools. Leaving the Robotiq collision meshes enabled creates many hard
  convex-convex self-collision pairs in MuJoCo Warp and can saturate CCD.
  """
  try:
    _disable_geom_contact(model.geom(f"{side}_hand_collision"))
  except KeyError:
    pass

  gripper_prefix = f"{side}_gripper/"
  for body in model.bodies:
    if not body.name or not body.name.startswith(gripper_prefix):
      continue
    for geom in body.geoms:
      _disable_geom_contact(geom)
