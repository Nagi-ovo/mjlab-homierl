...
def upper_body_action_curriculum(
  ...
  - Returns the ratio for logging. When episode statistics are available, also
    returns the computed average raw reward for debugging.
  """
  ...
  # Episode-average raw reward (per-step, unweighted) = accumulated / (episode_steps * dt * weight).
  # Use actual episode lengths so short (failed) episodes do not suppress the curriculum forever.
  ...
  if isinstance(env_ids, slice):
    episode_sums = reward_manager._episode_sums[reward_name]
    episode_steps = env.episode_length_buf.to(dtype=torch.float32)
  else:
    episode_sums = reward_manager._episode_sums[reward_name][env_ids]
    episode_steps = env.episode_length_buf[env_ids].to(dtype=torch.float32)

  episode_steps = torch.clamp(episode_steps, min=1.0)
  avg_raw_reward = torch.mean(episode_sums / (episode_steps * dt * weight))
  ...
  return {
    "value": action_term.curriculum_ratio.unsqueeze(0),
    "avg_raw_reward": avg_raw_reward,
    "episode_steps_mean": torch.mean(episode_steps),
  }
