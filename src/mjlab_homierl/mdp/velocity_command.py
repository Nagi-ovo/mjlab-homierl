...
class UniformVelocityCommand(CommandTerm):
  ...
  def _ensure_homie_mode_buffer(self) -> torch.Tensor:
    """Ensure the env has a buffer for HOMIE 3-mode command coupling.
    ...
    Mode encoding:
      ...
      2 = height/squat
    """
    mode = getattr(self._env, "_homie_command_mode", None)
    if mode is None or not isinstance(mode, torch.Tensor) or mode.shape != (self.num_envs,):
      mode = torch.zeros(self.num_envs, device=self.device, dtype=torch.int64)
      setattr(self._env, "_homie_command_mode", mode)
    return mode
  ...
  def _resample_command(self, env_ids: torch.Tensor) -> None:
    ...
    if self.cfg.homie_three_mode:
      # IsaacGym HOMIE behavior: mutually exclusive modes at command resampling:
      #   - 1/3: height/squat (twist command is zero)
      #   - 1/2: velocity tracking (sample twist command)
      #   - 1/6: standing (twist command is zero)
      #
      # This term acts as the "source of truth" for mode selection and stores
      # it on the environment so the height command can couple to the same
      # resampling.
      set_x = torch.rand((len(env_ids), 1), device=self.device)
      is_height = set_x < (1.0 / 3.0)
      is_vel = set_x > (1.0 / 2.0)
      is_height = is_height.squeeze(1)
      is_vel = is_vel.squeeze(1)

      vel_env_ids = env_ids[is_vel]
      non_vel_env_ids = env_ids[~is_vel]

      if vel_env_ids.numel() > 0:
        r = torch.empty(len(vel_env_ids), device=self.device)
        self.vel_command_b[vel_env_ids, 0] = r.uniform_(*self.cfg.ranges.lin_vel_x)
        self.vel_command_b[vel_env_ids, 1] = r.uniform_(*self.cfg.ranges.lin_vel_y)
        self.vel_command_b[vel_env_ids, 2] = r.uniform_(*self.cfg.ranges.ang_vel_z)

      if non_vel_env_ids.numel() > 0:
        self.vel_command_b[non_vel_env_ids, :] = 0.0

      # Disable heading behavior in this mode; commands are direct (x, y, yaw).
      self.heading_target[env_ids] = 0.0
      self.heading_error[env_ids] = 0.0
      self.is_heading_env[env_ids] = False

      # Treat all non-velocity envs (height + standing) as standing for twist.
      self.is_standing_env[env_ids] = ~is_vel

      # Publish coupled mode to the environment for the height command.
      mode = self._ensure_homie_mode_buffer()
      mode[env_ids] = 0  # standing default
      mode[env_ids[is_vel]] = 1
      mode[env_ids[is_height]] = 2
      return
    ...
    # Sample standing (zero velocity) with probability rel_standing_envs
    new_standing = r.uniform_(0.0, 1.0) <= self.cfg.rel_standing_envs
    # If avoid_consecutive_standing is enabled, don't sample standing if already standing
    if self.cfg.avoid_consecutive_standing:
      was_standing = self.is_standing_env[env_ids]
      new_standing = new_standing & (~was_standing)
    self.is_standing_env[env_ids] = new_standing

    if (self.cfg.rel_pure_lin_vel_x_envs > 0.0) or (self.cfg.rel_pure_lin_vel_y_envs > 0.0):
      move_prob = 1.0 - float(self.cfg.rel_standing_envs)
      if move_prob > 0.0:
        non_standing_env_ids = env_ids[~self.is_standing_env[env_ids]]
        if non_standing_env_ids.numel() > 0:
          pure_x_cond = float(self.cfg.rel_pure_lin_vel_x_envs) / move_prob
          pure_y_cond = float(self.cfg.rel_pure_lin_vel_y_envs) / move_prob
          u = torch.rand(len(non_standing_env_ids), device=self.device)
          pure_x = u < pure_x_cond
          pure_y = (u >= pure_x_cond) & (u < (pure_x_cond + pure_y_cond))
          if pure_x.any():
            self.vel_command_b[non_standing_env_ids[pure_x], 1] = 0.0
          if pure_y.any():
            self.vel_command_b[non_standing_env_ids[pure_y], 0] = 0.0
...
class UniformVelocityCommandCfg(CommandTermCfg):
  ...
  rel_pure_lin_vel_x_envs: float = 0.0
  rel_pure_lin_vel_y_envs: float = 0.0
  init_velocity_prob: float = 0.0
  active_env_group: str | None = None
  avoid_consecutive_standing: bool = False
  homie_three_mode: bool = False
  """If True, resample commands using IsaacGym HOMIE mutually exclusive 3-mode scheme.

  Modes are sampled at each resampling instant:
    - 1/3: height/squat (velocity command forced to zero)
    - 1/2: velocity tracking (sample velocity commands)
    - 1/6: standing (velocity command forced to zero)

  When enabled, this term writes the sampled mode into ``env._homie_command_mode``
  so that the coupled height command can use the same mode partition.
  """
  ...
  def __post_init__(self):
    ...
    if self.rel_pure_lin_vel_x_envs < 0.0 or self.rel_pure_lin_vel_y_envs < 0.0:
      raise ValueError("Pure-axis sampling probabilities must be >= 0.")
    if (self.rel_pure_lin_vel_x_envs + self.rel_pure_lin_vel_y_envs) > 1.0:
      raise ValueError(
        "Sum of pure-axis sampling probabilities must be <= 1. "
        f"(got rel_pure_lin_vel_x_envs={self.rel_pure_lin_vel_x_envs}, "
        f"rel_pure_lin_vel_y_envs={self.rel_pure_lin_vel_y_envs})."
      )
    if self.rel_standing_envs < 0.0 or self.rel_standing_envs > 1.0:
      raise ValueError("rel_standing_envs must be in [0, 1].")
    if (
      self.rel_standing_envs
      + self.rel_pure_lin_vel_x_envs
      + self.rel_pure_lin_vel_y_envs
    ) > 1.0:
      raise ValueError(
        "Sum of standing + pure-axis sampling probabilities must be <= 1. "
        f"(got rel_standing_envs={self.rel_standing_envs}, "
        f"rel_pure_lin_vel_x_envs={self.rel_pure_lin_vel_x_envs}, "
        f"rel_pure_lin_vel_y_envs={self.rel_pure_lin_vel_y_envs})."
      )
...
class RelativeHeightCommand(CommandTerm):
  ...
  def __init__(self, cfg: RelativeHeightCommandCfg, env: ManagerBasedRlEnv):
    ...
    # Target height command before optional smoothing. Shape: (num_envs, 1).
    self._height_target = torch.zeros_like(self.height_command)
  ...
  def _resample_command(self, env_ids: torch.Tensor) -> None:
    ...
    if self.cfg.active_env_group is not None:
      ...
      if group_mask is not None:
        ...
        if inactive_env_ids.numel() > 0:
          ...
          self._height_target[inactive_env_ids, 0] = inactive_h
    ...
    if self.cfg.homie_three_mode:
      # Couple height command to the HOMIE 3-mode selection performed by the
      # twist command (stored in env._homie_command_mode).
      mode = getattr(self._env, "_homie_command_mode", None)
      if mode is None or not isinstance(mode, torch.Tensor) or mode.shape != (self.num_envs,):
        # Fallback: if mode is missing, treat all envs as non-height.
        mode = torch.zeros(self.num_envs, device=self.device, dtype=torch.int64)
        setattr(self._env, "_homie_command_mode", mode)

      is_height_mode = mode[env_ids] == 2
      height_env_ids = env_ids[is_height_mode]
      non_height_env_ids = env_ids[~is_height_mode]

      if height_env_ids.numel() > 0:
        r = torch.empty(len(height_env_ids), device=self.device)
        self._height_target[height_env_ids, 0] = r.uniform_(*self.cfg.ranges.height)

      if non_height_env_ids.numel() > 0:
        inactive_h = (
          float(self.cfg.inactive_height)
          if self.cfg.inactive_height is not None
          else 0.5 * (self.cfg.ranges.height[0] + self.cfg.ranges.height[1])
        )
        self._height_target[non_height_env_ids, 0] = inactive_h

      # IsaacGym updates commands instantaneously at resampling.
      self.height_command[env_ids, 0] = self._height_target[env_ids, 0]
      return

    r = torch.empty(len(env_ids), device=self.device)
    self._height_target[env_ids, 0] = r.uniform_(*self.cfg.ranges.height)

    # Initialize to target on the first command (e.g., after reset) to avoid
    # large transient errors from smoothing starting at 0.
    if self.cfg.interp_rate >= 1.0:
      self.height_command[env_ids, 0] = self._height_target[env_ids, 0]
      return

    first_cmd_env_ids = env_ids[self.command_counter[env_ids] == 0]
    if first_cmd_env_ids.numel() > 0:
      self.height_command[first_cmd_env_ids, 0] = self._height_target[
        first_cmd_env_ids, 0
      ]

  def _update_command(self) -> None:
    # Optionally smooth height command transitions to avoid step changes.
    if self.cfg.interp_rate >= 1.0:
      return
    self.height_command[:, 0] = torch.lerp(
      self.height_command[:, 0], self._height_target[:, 0], self.cfg.interp_rate
    )
...
class RelativeHeightCommandCfg(CommandTermCfg):
  ...
  homie_three_mode: bool = False
  """If True, couple height resampling to HOMIE 3-mode command selection.

  The mode is expected to be written by the twist command into
  ``env._homie_command_mode`` (see ``UniformVelocityCommandCfg.homie_three_mode``).
  When enabled, height commands are resampled only for envs in height mode;
  other envs are forced to ``inactive_height``.
  """
  interp_rate: float = 1.0
  """Interpolation factor for smoothing height-command changes (per env-step).

  - ``1.0``: no smoothing (instant step to new command; previous default behavior).
  - ``0.0 < interp_rate < 1.0``: exponential smoothing via ``torch.lerp`` toward a
    per-resample target.
  """
  ...
  def __post_init__(self) -> None:
    if not (0.0 < self.interp_rate <= 1.0):
      raise ValueError(
        f"RelativeHeightCommandCfg.interp_rate must be in (0, 1], got {self.interp_rate}."
      )
