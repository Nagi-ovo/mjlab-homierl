...
class HomieLowerBodyPolicy:
  ...
  def __init__(self, checkpoint_path: Path, device: str = "cpu") -> None:
    ...
    if actor_weight_key is None:
      raise KeyError("Could not locate the exported actor weights in the checkpoint.")

    self.obs_dim = 97
    self.act_dim = int(sd[actor_weight_key].shape[0])
    ...
    self._mean = torch.zeros(self.obs_dim, dtype=torch.float32)
    self._std = torch.ones(self.obs_dim, dtype=torch.float32)
...
def _build_model_with_floor(xml_path: Path) -> mujoco.MjModel:
  ...
  # Override foot friction to (1.0, 0.005, 0.0001)
  foot_geom_names = [
  ...
  for name in foot_geom_names:
    ...
    if geom_id >= 0:
      model.geom_friction[geom_id] = [1.0, 0.005, 0.0001]
...
def main() -> None:
  ...
  parser.add_argument(
    ...
    help="Path to a HOMIE checkpoint (expects `policy_state_dict` or legacy `model_state_dict`).",
  )
  parser.add_argument(
    "--xml",
    type=Path,
